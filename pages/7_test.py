...
def prompt_generator(query, docs):
    prompt = f"""
    만약 사용자가 비속어나, 성적인 표현을 사용했다면 답변할 수 없다고 응답하세요.
    당신이 해야 할 일은 고등학생 사용자가 자신의 꿈이나 진로와 관련된 질문이나 이야기를 하면 응답을 기반으로 한 검색된 자료를 참고하여 답변을 생성해야 합니다.
    
    검색된 자료에는 다음과 같은 내용이 있습니다. : 학과이름, 학과소개, 학과주요교과목, 학과관련 고등학교 선택과목, 학과관련 추천 도서
    아래에 있는 검색 내용을 바탕으로 학과에 대해서 고등학생에게 소개하는 방식으로 설명해 주세요.
    """
